...
class InsertEndFors(ast.NodeTransformer):
    def visit_For(self, node):
        ...
        # if you don't insert a place holder some kind of early return
        # happens
...
def rewrite_bytecode(f):
    ...
    for i, c in enumerate(code):
        ...
        if c.name in {
            ...
            # affine_for returns an iterator with length 1
        }:
...
def mlir_func(
    ...
    **kwargs,
    ...
    if range_ctor is None:
        range_ctor = affine_for
    if func_ctor is None:
        func_ctor = MLIRFunc

    if rewrite_ast_:
        f = rewrite_ast(f, range_ctor=range_ctor)
