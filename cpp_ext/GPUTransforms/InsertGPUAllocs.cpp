...
namespace {
struct InsertGPUAllocsPass
    : public PassWrapper<InsertGPUAllocsPass, OperationPass<func::FuncOp>> {

  MLIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(InsertGPUAllocsPass)

  InsertGPUAllocsPass() = default;
  InsertGPUAllocsPass(const InsertGPUAllocsPass &pass) : PassWrapper(pass) {}

  void getDependentDialects(DialectRegistry &registry) const override {
    registry
        .insert<memref::MemRefDialect, gpu::GPUDialect, arith::ArithDialect>();
  }
  [[nodiscard]] StringRef getArgument() const final {
    return "insert-gpu-allocs";
  }

  Option<std::string> clientAPI{*this, "client-api", llvm::cl::init("vulkan"),
                                llvm::cl::Required};
  ...
  void runOnOperation() override {
    ...
    auto add_gpu_alloc = [this](mlir::OpBuilder builder, mlir::Value op,
      ...
      if (clientAPI == "opencl") {
        ...
        if (allocType != memrefType) {
          auto allocResult_ = builder.create<mlir::memref::CastOp>(
              loc, memrefType, allocResult);
          op.replaceAllUsesExcept(allocResult_, filter);
          builder.setInsertionPoint(term);
          if (access.hostRead && access.deviceWrite) {
            builder.create<mlir::memref::CopyOp>(loc, allocResult_, op);
          }
        } else {
          op.replaceAllUsesExcept(allocResult, filter);
          builder.setInsertionPoint(term);
...
namespace nelli {
void registerInsertGPUAllocsPass() { PassRegistration<InsertGPUAllocsPass>(); }
} // namespace nelli
