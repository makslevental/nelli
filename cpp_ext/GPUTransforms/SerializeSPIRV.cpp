...
struct SerializeSPIRVPass
    : public PassWrapper<SerializeSPIRVPass, OperationPass<mlir::ModuleOp>> {

  MLIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(SerializeSPIRVPass)

  void getDependentDialects(DialectRegistry &registry) const override {
    registry.insert<gpu::GPUDialect, spirv::SPIRVDialect>();
  }
  [[nodiscard]] StringRef getArgument() const final {
    return "serialize-spirv";
  }
...
namespace nelli {
void registerSerializeSPIRVPass() { PassRegistration<SerializeSPIRVPass>(); }
} // namespace nelli
