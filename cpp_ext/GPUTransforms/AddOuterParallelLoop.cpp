...
struct AddOuterParallelLoopPass
    : public PassWrapper<AddOuterParallelLoopPass,
                         OperationPass<func::FuncOp>> {
  MLIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(AddOuterParallelLoopPass)

  void getDependentDialects(DialectRegistry &registry) const override {
    registry.insert<scf::SCFDialect>();
  }
  [[nodiscard]] StringRef getArgument() const final {
    return "add-outer-parallel-loop";
  }
  ...
  void runOnOperation() override {
    ...
    for (auto ops : groupedOps) {
      ...
      auto outer = builder.create<scf::ParallelOp>(loc, cst0, cst1, cst1);
...
void registerAddOuterParallelLoopPass() {
  PassRegistration<AddOuterParallelLoopPass>();
}
} // namespace nelli
