...
static bool legalCondition(Value en, bool dim = false) {
  ...
  if (!dim && !isValidSymbolInt(en, /*recur*/ false)) {
    if (nelli::isValidIndex(en) || isValidSymbolInt(en, /*recur*/ true)) {
      return true;
    }
  }
    ...
    if (((!isValidSymbolInt(t, /*recur*/ false) &&
          (t.getDefiningOp<AddIOp>() || t.getDefiningOp<SubIOp>() ||
           (t.getDefiningOp<MulIOp>() &&
            ((nelli::isValidIndex(t.getDefiningOp()->getOperand(0)) &&
              isValidSymbolInt(t.getDefiningOp()->getOperand(1))) ||
             (nelli::isValidIndex(t.getDefiningOp()->getOperand(1)) &&
...
void nelli::fully2ComposeAffineMapAndOperands(PatternRewriter &builder, AffineMap *map,
                                       SmallVectorImpl<Value> *operands,
                                       DominanceInfo &DI) {
  IRMapping indexMap;
...
//         op.getOperation()->moveBefore(bop.getOwner(),
//         bop.getOwner()->begin()); return success();
//       }
//       return failure();
//     }
//
...
struct CanonicalizeAffineApply : public OpRewritePattern<AffineApplyOp> {
  ...
  LogicalResult matchAndRewrite(AffineApplyOp affineOp,
    ...
    nelli::fully2ComposeAffineMapAndOperands(rewriter, &map, &mapOperands, DI);
...
bool nelli::isValidIndex(Value val) {
  if (isValidSymbolInt(val))
    return true;

  if (auto cast = val.getDefiningOp<IndexCastOp>())
    return nelli::isValidIndex(cast.getOperand());
  ...
  if (auto bop = val.getDefiningOp<AddIOp>())
    return nelli::isValidIndex(bop.getOperand(0)) &&
           nelli::isValidIndex(bop.getOperand(1));
  ...
  LLVM_DEBUG(llvm::dbgs() << "illegal nelli::isValidIndex: " << val << "\n");
  return false;
}
...
// bool handle(PatternRewriter &b, CmpIOp cmpi, SmallVectorImpl<AffineExpr>
// &exprs,
...
*/
LogicalResult
nelli::MoveLoadToAffine::matchAndRewrite(memref::LoadOp load,
                                         PatternRewriter &rewriter) const {
  if (!llvm::all_of(load.getIndices(), nelli::isValidIndex)) {
    llvm::errs() << "invalid indices: " << load << "\n";
//    return failure();
  }
  ...
  if (map.getNumInputs() != operands.size()) {
    load->getParentOfType<func::FuncOp>().dump();
    llvm::errs() << "wtfbbq load: " << load << "\n";
  }
...
LogicalResult
nelli::MoveStoreToAffine::matchAndRewrite(memref::StoreOp store,
                                          PatternRewriter &rewriter) const {
  if (!llvm::all_of(store.getIndices(), nelli::isValidIndex)) {
    llvm::errs() << "invalid indices: " << store << "\n";
//    return failure();
  }
...
struct CanonicalieForBounds : public OpRewritePattern<AffineForOp> {
  ...
  LogicalResult matchAndRewrite(AffineForOp forOp,
    ...
    nelli::fully2ComposeAffineMapAndOperands(rewriter, &lbMap, &lbOperands, DI);
    canonicalizeMapAndOperands(&lbMap, &lbOperands);
    lbMap = removeDuplicateExprs(lbMap);

    nelli::fully2ComposeAffineMapAndOperands(rewriter, &ubMap, &ubOperands, DI);
...
//
//     auto iset =
//         IntegerSet::get(/*dim*/ 0, /*symbol*/ 2 * exprs.size(), exprs,
//         eqflags);
