/// originally from
/// https://github.com/llvm/Polygeist/blob/main/lib/polygeist/Passes/RaiseToAffine.cpp

#include "RaiseToAffine.h"
#include "AffineCFG.h"
...
struct ForOpRaising : public OpRewritePattern<scf::ForOp> {
  ...
  LogicalResult matchAndRewrite(scf::ForOp loop,
    ...
    if (isAffine(loop)) {
      ...
      {
        nelli::fully2ComposeAffineMapAndOperands(rewriter, &lbMap, &lbs, DI);
        canonicalizeMapAndOperands(&lbMap, &lbs);
        lbMap = removeDuplicateExprs(lbMap);
      }
      AffineMap ubMap = getMultiSymbolIdentity(builder, ubs.size());
      {
        nelli::fully2ComposeAffineMapAndOperands(rewriter, &ubMap, &ubs, DI);
...
struct ParallelOpRaising : public OpRewritePattern<scf::ParallelOp> {
  ...
  LogicalResult matchAndRewrite(scf::ParallelOp loop,
    ...
    if (!llvm::all_of(loop.getLowerBound(), nelli::isValidIndex)) {
      return failure();
    }

    if (!llvm::all_of(loop.getUpperBound(), nelli::isValidIndex)) {
      return failure();
    }
...
struct RaiseSCFToAffine
  ...
  RaiseSCFToAffine(const RaiseSCFToAffine &pass) : PassWrapper(pass) {}
  void getDependentDialects(DialectRegistry &registry) const override {
    registry.insert<AffineDialect, scf::SCFDialect>();
  }
  [[nodiscard]] StringRef getArgument() const final {
    return "raise-scf-to-affine";
  }

  Option<bool> forOps{*this, "for-ops", llvm::cl::init(false)};
  Option<bool> parallelOps{*this, "parallel-ops", llvm::cl::init(false)};
  Option<bool> storeOps{*this, "store-ops", llvm::cl::init(false)};
  Option<bool> loadOps{*this, "load-ops", llvm::cl::init(false)};
  ...
  void runOnOperation() override {
    ...
    if (parallelOps)
      patterns.insert<ParallelOpRaising>(&getContext());
    if (storeOps)
      patterns.insert<nelli::MoveStoreToAffine>(&getContext());
    if (loadOps)
      patterns.insert<nelli::MoveLoadToAffine>(&getContext());
...
namespace nelli {
void registerRaiseSCFToAffinePass() { PassRegistration<RaiseSCFToAffine>(); }
} // namespace nelli
