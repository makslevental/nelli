...
#include "StructuredTransformOpsExt.h"
#include "Transform/TransformMatchers.h"
...
mlir::transform::StructuredTransformOpsExtension::
    StructuredTransformOpsExtension() {
  registerTransformOps<
#define GET_OP_LIST
#include "StructuredTransformOpsExt.cpp.inc"
      >();
...
void mlir::registerTransformDialectStructuredExtension(
    DialectRegistry &registry) {
  registry.addExtensions<
      mlir::transform::StructuredTransformOpsExtension>();
}

void mlir::registerTransformDialectStructuredExtension(MLIRContext &context) {
  DialectRegistry registry;
  registerTransformDialectStructuredExtension(registry);
...
transform::LowerToLLVMOp::apply(mlir::transform::TransformResults &result,
...
DiagnosedSilenceableFailure transform::MatchCallbackOp::apply(
  ...
  auto *registry = state.getExtension<transform::MatchCallbacksRegistry>();
  if (!registry)
    return errorOut() << "match registry not available";

  const transform::MatchCallbacksRegistry::MatchCallbackFn *callback =
...
void transform::MatchCallbackOp::getEffects(
...
/// "test.transform_do_not_match" attribute, in which case produces a
/// silenceable failure.
static DiagnosedSilenceableFailure
testMatchCallbackCallback(transform::MatchCallbackResult &res, Location loc,
                          const mlir::transform::TransformState &state,
                          ValueRange handles) {
  bool hadFailures = false;
  for (Value handle : handles) {
    if (llvm::any_of(state.getPayloadOps(handle), [](Operation *op) {
          return op->hasAttr("test.transform_do_not_match");
        })) {
...
static DiagnosedSilenceableFailure testRepeatedMatcherUseCallback(
    transform::MatchCallbackResult &res, Location loc,
  ...
  auto &operand = transform::m_StructuredOp(matcherContext);
  auto &first = transform::m_StructuredOp(matcherContext).input(0, operand);
  auto &second = transform::m_StructuredOp(matcherContext)
                     .input(0, operand)
                     .input(1, first);
...
static DiagnosedSilenceableFailure
testValueMatcherCallback(transform::MatchCallbackResult &res, Location loc,
                         const mlir::transform::TransformState &state,
                         ValueRange handles) {
  if (handles.size() != 1 || state.getPayloadOps(handles[0]).size() != 1) {
    return emitSilenceableFailure(loc)
  ...
  transform::MatcherContext matcherContext;
  auto &operand = transform::m_Value(matcherContext);
  auto &first = transform::m_StructuredOp(matcherContext).input(0, operand);
  auto &second = transform::m_StructuredOp(matcherContext)
                     .input(0, operand)
                     .input(1, first);
...
static DiagnosedSilenceableFailure
reductionCallback(transform::MatchCallbackResult &res, Location loc,
                  const mlir::transform::TransformState &state,
                  ValueRange handles) {
  if (handles.size() != 1 || state.getPayloadOps(handles[0]).size() != 1) {
    return emitSilenceableFailure(loc)
  ...
  transform::StructuredOpMatcher *pattern, *fill, *leading, *trailing;
  transform::MatchedReductionCaptures ignore;
  transform::MatcherContext matcherContext;
...
DiagnosedSilenceableFailure transform::RegisterMatchCallbacksOp::apply(
    mlir::transform::TransformResults &results,
    mlir::transform::TransformState &state) {
  auto &registry = state.addExtension<transform::MatchCallbacksRegistry>();
...
void transform::RegisterMatchCallbacksOp::getEffects(
    SmallVectorImpl<MemoryEffects::EffectInstance> &effects) {
  // TODO: it doesn't really modify the payload, we need a separate resource for
  // this mapping.
  mlir::transform::modifiesPayload(effects);
...
DiagnosedSilenceableFailure
transform::TakeFirstOp::apply(mlir::transform::TransformResults &results,
                                  mlir::transform::TransformState &state) {
...
void transform::TakeFirstOp::getEffects(
    SmallVectorImpl<MemoryEffects::EffectInstance> &effects) {
  mlir::transform::onlyReadsHandle(getInputs(), effects);
...
DiagnosedSilenceableFailure transform::EmitRemarkOp::applyToOne(
...
void transform::EmitRemarkOp::getEffects(
    SmallVectorImpl<MemoryEffects::EffectInstance> &effects) {
