...
void transform::CapturingOpMatcher::getAllNested(
...
void transform::CapturingOpMatcher::getAllNestedValueMatchers(
...
bool transform::ValueMatcher::match(Value value) {
  auto debugRAII =
...
transform::StructuredOpMatcher &
transform::StructuredOpMatcher::rank(NumGreaterEqualTo minRank) {
...
transform::StructuredOpMatcher &
transform::StructuredOpMatcher::rank(NumLowerEqualTo maxRank) {
  predicates.push_back([=](linalg::LinalgOp linalgOp) -> bool {
...
StringRef stringifyShapeKind(transform::ShapeKind kind) {
  switch (kind) {
  case transform::ShapeKind::Static:
    return "static";
  case transform::ShapeKind::Dynamic:
    return "dynamic";
  }
...
transform::StructuredOpMatcher &
transform::StructuredOpMatcher::dim(SmallVector<int64_t> &&dimensions,
                                        ShapeKind kind) {
...
transform::StructuredOpMatcher &
transform::StructuredOpMatcher::dim(AllDims tag, ShapeKind kind) {
  predicates.push_back([=](linalg::LinalgOp linalgOp) -> bool {
...
transform::StructuredOpMatcher &
transform::StructuredOpMatcher::dim(AllDims tag, utils::IteratorType kind) {
  return dim(AllDimsExcept({}), kind);
}

transform::StructuredOpMatcher &
transform::StructuredOpMatcher::dim(AllDimsExcept &&dims,
                                        utils::IteratorType kind) {
...
transform::StructuredOpMatcher &
transform::StructuredOpMatcher::dim(int64_t dimension,
...
//===---------------------------------------------------------------------===//
transform::StructuredOpMatcher &
transform::StructuredOpMatcher::rank(CaptureRank capture) {
  predicates.push_back([=](linalg::LinalgOp linalgOp) -> bool {
...
transform::StructuredOpMatcher &
transform::StructuredOpMatcher::dim(int64_t dimension, CaptureDim capture) {
  predicates.push_back([=](linalg::LinalgOp linalgOp) -> bool {
...
transform::StructuredOpMatcher &
transform::StructuredOpMatcher::dim(AllDims tag, CaptureDims captures) {
  predicates.push_back([=](linalg::LinalgOp linalgOp) -> bool {
...
transform::StructuredOpMatcher::StructuredOpMatcher(
...
void transform::StructuredOpMatcher::addInputMatcher(
...
transform::StructuredOpMatcher &
transform::StructuredOpMatcher::input(AllOperands tag, IsPermutation) {
  predicates.push_back([=](linalg::LinalgOp linalgOp) -> bool {
...
transform::StructuredOpMatcher &
transform::StructuredOpMatcher::input(AllOperands tag,
...
transform::StructuredOpMatcher &
transform::StructuredOpMatcher::input(SmallVector<int64_t> &&positions,
                                          IsProjected dim) {
  predicates.push_back([=](linalg::LinalgOp linalgOp) -> bool {
...
transform::StructuredOpMatcher &
transform::StructuredOpMatcher::input(AllOperands tag, IsIdentity) {
  predicates.push_back([=](linalg::LinalgOp linalgOp) -> bool {
...
transform::StructuredOpMatcher &
transform::StructuredOpMatcher::input(int64_t position,
...
transform::StructuredOpMatcher &
transform::StructuredOpMatcher::input(NumEqualsTo num) {
  predicates.push_back([=](linalg::LinalgOp linalgOp) -> bool {
...
transform::StructuredOpMatcher &
transform::StructuredOpMatcher::input(int64_t position, ConstantFloatZero) {
  return input(position, [](llvm::APFloat f) { return f.isZero(); });
}

transform::StructuredOpMatcher &transform::StructuredOpMatcher::input(
...
void transform::StructuredOpMatcher::addOutputMatcher(
    int64_t position, std::function<bool(Operation *)> matcher,
    OptionalMatch optional) {
  predicates.push_back([position, optional, matcher = std::move(matcher)](
                           linalg::LinalgOp linalgOp) -> bool {
...
transform::StructuredOpMatcher &
transform::StructuredOpMatcher::output(AllOperands tag, IsPermutation) {
  predicates.push_back([=](linalg::LinalgOp linalgOp) -> bool {
...
transform::StructuredOpMatcher &
transform::StructuredOpMatcher::output(AllOperands tag,
                                           IsProjectedPermutation) {
  predicates.push_back([=](linalg::LinalgOp linalgOp) -> bool {
...
transform::StructuredOpMatcher &
transform::StructuredOpMatcher::output(AllOperands tag, IsProjected dim) {
  predicates.push_back([=](linalg::LinalgOp linalgOp) -> bool {
...
transform::StructuredOpMatcher &
transform::StructuredOpMatcher::output(AllOperands tag, IsIdentity) {
  predicates.push_back([=](linalg::LinalgOp linalgOp) -> bool {
...
transform::StructuredOpMatcher &
transform::StructuredOpMatcher::output(int64_t position,
                                           ElementTypeBitWidth width) {
  predicates.push_back([=](linalg::LinalgOp linalgOp) -> bool {
    LLVM_DEBUG(DBGS() << "output operand #" << position
                      << " has elemental type with bit width " << width.value);
...
transform::StructuredOpMatcher &
transform::StructuredOpMatcher::output(NumEqualsTo num) {
  predicates.push_back([=](linalg::LinalgOp linalgOp) -> bool {
...
void transform::StructuredOpMatcher::addResultMatcher(
...
bool transform::StructuredOpMatcher::checkAllTilableMatched(
    Operation *parent, linalg::LinalgOp linalgOp,
    ArrayRef<transform::CapturingOpMatcher *> matchers) {
...
transform::StructuredOpMatcher &
transform::StructuredOpMatcher::singleOpWithCanonicaleArgs(
...
transform::StructuredOpMatcher &
transform::StructuredOpMatcher::isFloatReciprocal() {
  predicates.push_back([=](linalg::LinalgOp linalgOp) {
...
transform::StructuredOpMatcher &
transform::StructuredOpMatcher::passThroughOp() {
  predicates.push_back([=](linalg::LinalgOp linalgOp) {
...
ArrayRef<Operation *>
transform::MatchCallbackResult::getPayloadGroup(int64_t position) const {
...
void transform::makeReductionMatcher(
    transform::MatcherContext &matcherContext,
    transform::StructuredOpMatcher *&reductionCapture,
    transform::StructuredOpMatcher *&fillCapture,
    transform::StructuredOpMatcher *&leadingCapture,
    transform::StructuredOpMatcher *&trailingCapture,
  ...
  //
  transform::StructuredOpMatcher commonLeadingOrTrailing =
...
void transform::makeReductionMatcher(transform::MatcherContext &context,
...
static void matchSubBroadcat(transform::MatcherContext &matcherContext,
                             transform::StructuredOpMatcher &maxReduction,
                             transform::ValueMatcher &softmaxSourceOperand,
                             transform::StructuredOpMatcher *&sub) {
  using namespace transform;

  auto &broadcast =
      transform::m_StructuredOp<linalg::GenericOp>(matcherContext)
          .passThroughOp()
          .dim(AllDims(), utils::IteratorType::parallel)
          .input(NumEqualsTo(1))
  ...
  auto &subOr = transform::m_StructuredOp_Or(matcherContext, subBroadcast,
                                                 subParallel);
  sub = &subOr;
}

/// Match sum(%exp, broadcast(%sum))
static void matchdivBroadcast(transform::MatcherContext &matcherContext,
                              transform::StructuredOpMatcher &expOperand,
                              transform::StructuredOpMatcher &sum,
                              transform::StructuredOpMatcher *&div) {
  using namespace transform;
  ...
  auto &divMerge = transform::m_StructuredOp_Or(
      matcherContext, divNoBroadcast, divBroadcast);
  div = &divMerge;
}

void transform::makeSoftmaxMatcher(
    transform::MatcherContext &matcherContext,
    transform::StructuredOpMatcher *&maxReductionCapture,
    transform::StructuredOpMatcher *&softmaxRootCapture) {
  ...
  transform::StructuredOpMatcher *subOperand;
  ...
  transform::StructuredOpMatcher *divOperand;
  matchdivBroadcast(matcherContext, expOperand, sum, divOperand);

  auto &softmaxRoot =
      transform::m_StructuredOp_Or(matcherContext, mulOperand, *divOperand);
